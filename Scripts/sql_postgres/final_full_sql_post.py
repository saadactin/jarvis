"""
Full SQL Server to PostgreSQL Migration Tool with Scheduling
This script performs complete migration of all data from SQL Server to PostgreSQL.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('final_full_sync.log'),
...
def full_sync_table(pg_engine, server_conf, db_name, server_clean, sql_engine, conn, schema, table):
    """Perform full sync of a table from SQL Server to PostgreSQL"""
    ...
    try:
        ...
        if schema_df.empty:
            logger.warning(f"No schema information found for {schema}.{table}")
            return 0
        ...
        # Create schema and table in PostgreSQL
        ...
        if not df.empty:
            # Truncate existing data and insert fresh
            with pg_engine.connect() as pg_conn:
                pg_conn.execute(text(f'TRUNCATE TABLE "{schema_name}"."{table_name}"'))
                pg_conn.commit()
            
            df.to_sql(table_name, pg_engine, schema=schema_name, if_exists='append', index=False, chunksize=BATCH_SIZE)
            
            # Update last synced PK
            ...
            logger.info(f"Table {schema}.{table} is empty, but schema created in PostgreSQL")
            return 0
            
    except Exception as e:
        logger.error(f"Failed to process table {schema}.{table}: {e}")
        return 0

def full_sync_database(sql_engine, db_name, server_conf, server_clean, pg_engine):
    """Perform full sync of a database"""
    logger.info(f"=== Starting FULL sync for database: {db_name} ===")
    ...
    logger.info(f"Found {len(tables)} tables for FULL sync")
    processed_count = 0
    
    for i, (schema, table) in enumerate(tables, 1):
        try:
            logger.info(f"[FULL SYNC] Processing {schema}.{table} ({i}/{len(tables)})")
    ...
    logger.info(f"=== FULL sync completed for {db_name}, {processed_count}/{len(tables)} tables processed ===")
    return processed_count

def process_sql_server_full(server_name, server_conf):
    """Process full sync for a SQL Server"""
    try:
        if SIMPLE_TERMINAL:
            print(f"=== FULL SYNC STARTED for server: {server_name} ===", flush=True)
        logger.info(f"FULL SYNC STARTED for {server_name}")
        
        if not SIMPLE_TERMINAL:
            print(f"[INIT] Initializing full sync for {server_name}...", flush=True)
        ...
        for db_name in databases:
            ...
            try:
                if not SIMPLE_TERMINAL:
                    print(f"[FULL SYNC] Performing FULL sync for {db_name}", flush=True)
                
                processed = full_sync_database(sql_engine, db_name, server_conf, server_clean, pg_engine)
        ...
        if SIMPLE_TERMINAL:
            print(f"\n=== FULL SYNC COMPLETE for server: {server_name} ===", flush=True)
        ...
        logger.info(f"Completed full sync for {server_name}")
...
def schedule_interval_sync(server_name, minutes):
    """Schedule interval-based full sync"""
    config = load_config()
    server_conf = config['sqlservers'].get(server_name)
    if not server_conf:
        raise ValueError(f"Server {server_name} not found in config")
    
    job_type = f"full_interval_{minutes}m"
    
    schedule.every(minutes).minutes.do(
        process_sql_server_full, server_name, server_conf
    ).tag(server_name, job_type)
    
    logger.info(f"Scheduled full sync for {server_name} every {minutes} minutes")
...
def schedule_daily_sync(server_name, hour, minute):
    ...
    job_type = f"full_daily_{time_str}"
    
    schedule.every().day.at(time_str).do(
        process_sql_server_full, server_name, server_conf
    ).tag(server_name, job_type)
    
    logger.info(f"Scheduled daily full sync for {server_name} at {time_str}")
    _start_scheduler_thread()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Full SQL Server to PostgreSQL Migration Tool')
    ...
    if args.schedule_interval and args.server:
        ...
        print(f"✅ Scheduled full sync for {args.server} every {args.schedule_interval} minutes")
    ...
    if args.schedule_daily and args.server:
        try:
            ...
            print(f"✅ Scheduled daily full sync for {args.server} at {args.schedule_daily}")
            args.run_scheduler = True
        except ValueError:
    ...
    if args.all:
        ...
        if args.server not in sqlservers:
            ...
            return
        process_sql_server_full(args.server, sqlservers[args.server])
    else:
        parser.print_help()
